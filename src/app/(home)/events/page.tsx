...
function EventPage() {
  ...
  async function onSubmit() {
    try {
      const latitude = comLocation.geometry.location.lat;
      const longitude = comLocation.geometry.location.lng;
        ...
        <div className="relative bg-gradient-to-r from-primaryColor via-50% via-[#77C574] to-primaryColor md:h-[409px] h-[274px]">
          ...
          <div className="absolute top-0 left-0 right-0 bottom-0 flex max-w-[1280px] m-auto justify-between px-4">
            <div className="content-center space-y-5">
              <p className="font-[800] lg:text-[80px] lg:leading-[80px] md:text-[60px] md:leading-[60px] mmd:text-[45px] mmd:leading-[45px] text-[36px] leading-[36px] text-white">
                LIST YOUR EVENT
              </p>
        ...
        <div className="bg-[#f3f3f6] py-20">
          <div className="max-w-[950px] lg:m-auto mx-4 bg-white pt-10 px-5 rounded-[6px]">
            <div className=" gap-5 pb-5">
              <div>
                <p className="text-[#000000] font-[700] mmd:text-[30px] text-[21px] mb-1">
                  List your event
                </p>
                <p className="text-[#000000] font-[400] mmd:text-[17px] text-[14px] mmd:mb-6 mb-3">
                  Here’s a list of upcoming events by our band in different
                  locations. Please choose a location near to you.
                </p>
              </div>
              ...
              <div className="mmd:grid grid-cols-1 mmd:space-y-0 gap-5 space-y-5 w-full py-4">
                ...
                <Options
                  ...
                  setPosition={(p) => {
                    console.log(p);
                    setVenueAddress(p.formatted_address);
                    setComLocation(p);
                    let x = [];
                    setLocations(x);
                  }}
              ...
              <div className="mmd:grid grid-cols-1 mmd:space-y-0 gap-5 space-y-2 w-full mt-5 mb-5">
                <label>
                  <input
                    type="checkbox"
                    className="mr-3"
                    onClick={() => setTicketForSel(!isTicketForSel)}
                  />
                ...
                {isTicketForSel && (
                  <div className="transition transition-display">
                    {tickets.map((ticket, index) => (
                      <div
                        className="flex flex-row justify-around gap-3 p-5"
                        key={index}
                      >
                        <input
                          type="string"
                          className="w-full bg-white mt-0 mmd:h-[64px] h-[45px] border rounded-md p-[22px] text-[17px] disabled"
                          value={ticket.type}
                        />
                        <input
                          type="string"
                        ...
                        <button
                          type="button"
                          onClick={() => removeTicketList(ticket.type)}
                        >
                          <FaRegSquareMinus />
                        </button>
                      </div>
                    ...
                    <div className="flex flex-row justify-around gap-3 p-5">
                      ...
                      <input
                        ...
                        value={ticektLimit}
                      />
                      <button type="button" onClick={addTicketList}>
                        <FaRegSquarePlus />
                      </button>
                    </div>
                  </div>
              ...
              <div className="mmd:grid grid-cols-1 mmd:space-y-0 gap-5 space-y-2 w-full mb-5">
                ...
                />
                <div
                  className="w-full bg-white mt-0 mmd:h-[64px] h-[45px] border rounded-md p-[22px] text-[17px] flex flex-row justify-between"
                  onClick={() => fileInputRef.current.click()}
                >
                  <div>{imgFile ? imgFileName : "Upload Event Flyer"}</div>
                  <div>
                    <FaLink />
                  </div>
                </div>
              ...
              <div className="flex flex-row justify-center mt-5">
                <button
                  type="button"
                  className="w-[206px] text-white bg-[#000000] py-[27px] px-[40px] rounded-[4px] mr-[5px] text-[15px]"
                  onClick={() => alert("ok")}
                >
                  SAVE FOR LATER
                </button>
                <button
                  type="button"
                  className="w-[206px] text-white bg-[#34A853] py-[27px] px-[40px] rounded-[4px] ml-[5px] text-[15px]"
                  onClick={onSubmit}
